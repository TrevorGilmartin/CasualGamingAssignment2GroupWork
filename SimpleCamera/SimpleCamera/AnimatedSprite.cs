...
namespace Sprites
{
    public class AnimeSprite
    {
        ...
        public AnimeSprite(Texture2D texture,Vector2 userPosition, int framecount)//constructor
        {
            spriteImage = texture;
        ...
        public bool collisionDetect(AnimeSprite otherSprite)
        {
        ...
        public virtual void Draw(SpriteBatch spriteBatch , SimpleCam cam)
        {
            //draw the sprite , specify the postion and source for the image withtin the sprite sheet
            //spriteBatch.Begin();
            // Changed to allow for sprite effect
            spriteBatch.Draw(spriteImage, cam.GetCameraCoordinates(position), sourceRectangle, Color.White, 0f, Vector2.Zero, 1.0f, _effect, 0f);
            if (ShowRect != null)
                spriteBatch.Draw(ShowRect, BoundingRect, Color.White);
           // spriteBatch.End();
        }
